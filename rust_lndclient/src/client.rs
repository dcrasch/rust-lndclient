...
use grpc::{ClientStub, MetadataKey};
...
unsafe impl Sync for rpc::Invoice {
}
...
impl LndClient {
    ...
    pub fn subscribe_invoices(
        ...
        add_index: u64,
        settle_index: u64,
    ) -> grpc::StreamingResponse<rpc::Invoice> {
        let options = self.request_options();
        let mut req = rpc::InvoiceSubscription::new();
        let resp = self.lightningclient.subscribe_invoices(options, req);
        resp
    }
}
